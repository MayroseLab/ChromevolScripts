...
my ($thresPP, $thresDP) = @{chromevol::determine_thresholds_for_power($inferDir,$thresFilePP, $thresFileDP, $ploidyCallType)}
...
chromevol::compute_reliability_from_sim_for_power($inferDir,$thresDP, $thresPP, $ploidyCallType,$relSim,$inferReliabilityRef)
	or die "ERROR: Failed to compute reliability from simulations\n";

## Summarize reliability
# find counts edit file
opendir(DIR,$workdir);
my @countsEdit = grep {/counts_edit$/} readdir(DIR);
