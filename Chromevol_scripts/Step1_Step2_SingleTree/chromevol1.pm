...
sub return_arr_GoodSims{
	my ($GoodSimFile) = @_;
	
	open my $handle, '<', $GoodSimFile;
...
sub prepare_initial_run{
	...
	my @modelsList;
	if ($runModels eq "ALL"){
		@modelsList = ("DysDup","DysDupDem*","DysDupDem","Dys",
	...
	sub parseParamFile{
		...
		if ($_[0] =~ m/DysBnum/){
			if ($baseNum != 0){	# if user specifies base number
				s/<DysBnum>/$baseNum/;
				s/<OPT_DysBnum>/0/;	# don't optimize base number
			}
			else{
				...
				s/<DysBnum>/$minInData/;
				s/<OPT_DysBnum>/1/;	# optimize base number
			}
		}
		if ($root != 0){
...
sub parse_chromevol_results{
	...
	### RUN
		...
		no warnings;
		my @models = ('DysDup', 'DysDupDem*', 'DysDupDem', 'Dys', 'LINEAR_RATE', 'LINEAR_RATE_DEMI', 'LINEAR_RATE_DEMI_EST', 'LINEAR_RATE_NO_DUPL','DysBnum', 'DysBnumDup');
		my @modelInfo = ();
		foreach(@models){
	...
	sub parseChromEvolResults{
		...
		while(<FILE>){
			...
			elsif (m/^DysBnumBER_R\s+(.+)/ ){
				$baseNumR = $1;
			}
			elsif (m/^DysBnumBER\s+(.+)/ ){
				$baseNum = $1;
			}
...
sub compute_reliability_from_sim{
	...
	foreach my $n (@goodSimsList){
		...
		my $SIM_FILE = $inDirSim.'/dir_sim_1/'.$n.'/simEvents.txt';
...
sub read_input{
	...
	my $countsFile; my $inTreesFile; my $infTreeNum = 1;	# MD - changed from ->  my $infTreeNum = 100;
...
#LEAF    GAIN    LOSS    DUPLICATION     DEMI-DUPLICATION	(DysBnumBER)
...
sub get_params_distribution{
	...
	print $ofh "GAIN_DysDup\t".summarize_dist(\@gainConst)."\n" if (scalar(@gainConst)== 100);
	print $ofh "LOSS_DysDup\t".summarize_dist(\@lossConst)."\n" if (scalar(@lossConst)== 100);
...
sub summarize_models{
	...
	if (not exists $models{'Dys'}){
		$models{'Dys'} = 0;
	}
	if (not exists $models{'LINEAR_RATE_NO_DUPL'}){
		$models{'LINEAR_RATE_NO_DUPL'} = 0;
	}
	my $noDuplModels = $models{'Dys'} + $models{'LINEAR_RATE_NO_DUPL'};
...
# given a threshold, the function returns a hash with the total
# numbers of TP,TN,FP,FN in all simulations
# MichalD: this will calc the threshold using the inference from simulations of the same single tree
sub test_threshold_for_power_SameTreeSim{
	my $inferDir = $_[0];
	my $ploidyCallType = $_[1];
	my $thres = $_[2];	# threshold value to be tested
	my $type = $_[3];	# 0 ('under') for DP, 1 ('above') for PP
	
	my @GoodSimNumbers = return_arr_GoodSims("$inferDir/infer_tree_1/simulation/GoodSimulations");
	# get directories for each phylogeny
	opendir(DIR, $inferDir) or die $!;
	# Now we take the results from all the sim_infer_XX dirs at:
	#.../Bulbophyllum_Chromevol_prune/chromevol_out/infer/infer_tree_1/simulation
	#my @dirs = grep {-d "$inferDir/$_" && ! /^\./} readdir(DIR);
	my @dirs = grep {-d "$inferDir/infer_tree_1/simulation/$_" && ! /^\./} readdir(DIR);
	
	my %testResults = ("TP"=>0, "TN"=>0, "FP"=>0, "FN"=>0);	
	### iterate through all simulations
	# MichalD: need to iterate all simulations under the same treeDir *dir_sim
	#foreach my $dir (@dirs){
	foreach my $SimNum (@GoodSimNumbers){
		# check if can work with this phylogeny
		#my $completedFile = "$inferDir/$dir/inference_step_complete";
		#next unless -e $completedFile;
		
		# find exp file - ../infer/infer_tree_1/simulation/sim_infer_10/DysDup
		my $simInferDir = "$inferDir/infer_tree_1/simulation/sim_infer_$SimNum";
		opendir(SID,$simInferDir) or die $!;
		my @modelInferDir = grep {-d "$simInferDir/$_" && ! /^\./} readdir(SID); # array with one directory
		my $INF_FILE = "$simInferDir/$modelInferDir[0]/expectations.txt";
		close SID;
		# find sim events file
		my $simulationDir = "$inferDir/infer_tree_1/simulation";
		opendir(SD, $simulationDir) or die $!;
		my @simDir = grep {-d "$simulationDir/$_" && /^dir_sim/} readdir(SD);
		my $SIM_FILE = "$simulationDir/dir_sim_1/$SimNum/simEvents.txt";
		close SD;
		next unless -e $INF_FILE;
		my $INF_EXP = &parseExpectationsFile($INF_FILE, $thres, $ploidyCallType);		
		my $SIM_EXP = &parseExpectationsFile($SIM_FILE, 1, $ploidyCallType);
...
# Based on simulated data, this unction calculates the optimal thresholds
# for calling diploids and polyploids (two different thresholds)
sub determine_thresholds_for_power_OneTree{
	my $inferDir = $_[0];
	my $outFilePP = $_[1];
	my $outFileDP = $_[2];
	my $ploidyCallType = $_[3];
	...
	## determine PP threshold
	my $thresPP = prevPP_determine_PP_threshold_for_power_OneTree($inferDir,$outFilePP, $ploidyCallType);
	## determine DP threshold
	my $thresDP = prevDP_determine_DP_threshold_for_power_OneTree($inferDir,$outFileDP, $ploidyCallType, $thresPP);
...
# Based on simulated data, this unction calculates the optimal thresholds
# for calling diploids and polyploids (two different thresholds)
sub determine_thresholds_for_power_OneTreeMoreSim{ #100sim for one tree
	my $inferDir = $_[0];
	my $outFilePP = $_[1];
	my $outFileDP = $_[2];
	my $ploidyCallType = $_[3];
	...
	## determine PP threshold
	my $thresPP = determine_PP_threshold_for_power_OneTree($inferDir,$outFilePP, $ploidyCallType);
	## determine DP threshold
	my $thresDP = determine_DP_threshold_for_power_OneTree($inferDir,$outFileDP, $ploidyCallType, $thresPP);
...
# sub determine Polyploid threshold
# determines the optimal threshold for calling polyploids
sub determine_PP_threshold_for_power_OneTree{
	my $inferDir = $_[0];
	my $outFile = $_[1];
	my $ploidyCallType = $_[2];
	open (my $thresfh, "> $outFile") or die "Can't open output thresholds file $outFile";
	...
	my %thresMCC;
	for(my $thresP = 0; $thresP < 1.01; $thresP+=0.01){
		my $thresResultsRef = test_threshold_for_power_SameTreeSim($inferDir,$ploidyCallType, $thresP,1);
		my %thresResults = %{$thresResultsRef};
		my ($thresTP, $thresTN, $thresFP, $thresFN) = 
			($thresResults{"TP"}, $thresResults{"TN"}, $thresResults{"FP"}, $thresResults{"FN"});
		my $mcc = MCC($thresTP, $thresTN, $thresFP, $thresFN);
...
# sub determine Polyploid threshold
# determines the optimal threshold for calling polyploids
sub prevPP_determine_PP_threshold_for_power_OneTree{
	my $inferDir = $_[0];
	my $outFile = $_[1];
	my $ploidyCallType = $_[2];
	
	my $bestThresP = "0";
	open my $info, $outFile or die "Could not open $outFile: $!";

	while( my $line = <$info>)  {   
		chomp $line;
		if ($line =~ m/Best threshold\:\s(\d+\.?\d*)/){
				print $1;
				my $bestThresP = $1;
				close $info;
				return $bestThresP;			
	...
	close $info;
	die "Failed to find best threshold";
}

# sub determine Diploid threshold
# determines the optimal threshold for calling diploids
sub determine_DP_threshold_for_power_OneTree{
	my $inferDir = $_[0];
	my $outFile = $_[1];
	my $ploidyCallType = $_[2];
	my $thresP = $_[3];	# PP threshold given as upper limit of DP threshold
	...
	my %thresMCC;
	for(my $thresD = 0; $thresD < $thresP; $thresD+=0.01){
		my $thresResultsRef = test_threshold_for_power_SameTreeSim($inferDir,$ploidyCallType, $thresP,0);
		my %thresResults = %{$thresResultsRef};
		my ($thresTP, $thresTN, $thresFP, $thresFN) = 
			($thresResults{"TP"}, $thresResults{"TN"}, $thresResults{"FP"}, $thresResults{"FN"});
...
sub prevDP_determine_DP_threshold_for_power_OneTree{
	my $inferDir = $_[0];
	my $outFile = $_[1];
	my $ploidyCallType = $_[2];
	my $thresP = $_[3];	# PP threshold given as upper limit of DP threshold
	...
	while( my $line = <$info>)  {   
		chomp $line;
		if ($line =~ m/Best threshold\:\s(\d+\.?\d*)/){ ##Best threshold: 0.26
				print $1;
				my $bestThresD = $1;
				close $info;
				return $bestThresD;	
...
sub compute_reliability_from_sim_for_power_OneTree{
	### FIELD
	my ($inferDir,$thresDP, $thresPP, $ploidyCallType,$outFile,$inferExpRef) = @_;
...
#This function will return a list of simulations that passed 3 tests:
sub test_all_simulations{
			
	#In this case SimNum is SimIteration under the one simulation directory:
	#../infer/infer_tree_1/simulation/dir_sim_1/0 ...199
	my ($simNum,$maxCount,$minCount,$allSimDir) = @_;
	my $goodSimFile = $allSimDir.'/GoodSimulations';
	open OUT, ">$goodSimFile" or die "ERROR: failed to create $goodSimFile...";
	my @arrGoodSim;
	
	my $maxRange = 200;	# max range allowed in simulations

	my @goodSims; # simulations that passed all 3 tests
	my $goodSimCounter = 0; #Stop when you have 100 good sim
	#for(my $n = 1; $n <= $simNum*2; $n++){
	for(my $n = 0; $n < 200; $n++){
		my ($rangeTest, $ploidyTest, $overMaxTest) = (0,0,0);
		my $simDir = "$allSimDir/dir_sim_1/$n/";
		...
		if ($goodSimCounter < 100){
			...
			#return $n;
				...
				#system("rm -rf $allSimDir/dir_sim_$n");
				#system("rm -rf $allSimDir/param_sim_$n");
			}
		}
		else{
			#system("rm -rf $allSimDir/dir_sim_$n");
