...
if ($bestModel eq "Dys" or $bestModel eq "LINEAR_RATE_NO_DUPL"){
...
my @goodSim = chromevol::test_all_simulations($simNum, $maxInData, $minInData, $simulationDir);
...
if (@goodSim){	# if a good simulation was found
	...
	#	}
	#}
	foreach my $Sim_n (@goodSim) {
		#say $Sim_n;

		## prepare infer from sim
		my $goodSimDir = "$simulationDir/dir_sim_1/$Sim_n";
		my $simCountsFile = "$goodSimDir/simCounts.txt";
		my $simInferDir = "$simulationDir/sim_infer_$Sim_n";
		mkdir $simInferDir;
		...
		## Generate jobs and send to queue:
		my $runFile = "$simInferDir/param_$bestModel\.txt";
		
		my $simInfer_sh = "$simInferDir/$Sim_n\_infer.sh";
		open OUT, ">$simInfer_sh" or die "ERROR: failed to create $simInfer_sh...";
		print OUT "#!/bin/tcsh\n";
		print OUT "#\$ -N simInfer_$Sim_n\n";
		print OUT "#\$ -S /bin/tcsh\n";
		print OUT "#\$ -cwd\n";
		#print OUT "#\$ -l h=!(compute-7-1)\n";
		print OUT "#\$ -e $simInferDir/simInfer_$Sim_n.ER\n";
		print OUT "#\$ -o $simInferDir/simInfer_$Sim_n.OU\n\n";
		print OUT "$chromEvolExe $runFile\n";
		close OUT;
		system ("qsub -l igv_low -p -2 $simInfer_sh") == 0
			or die "ERROR: Can't send $simInfer_sh to queue\n";
