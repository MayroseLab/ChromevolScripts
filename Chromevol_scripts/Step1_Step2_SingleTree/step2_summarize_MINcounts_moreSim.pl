...
my ($thresPP, $thresDP) = @{chromevol::determine_thresholds_for_power_OneTreeMoreSim($inferDir,$thresFilePP, $thresFileDP, $ploidyCallType)}
...
opendir(DIR,$workdir);
my @countsEdit = grep {/counts_min_edit$/} readdir(DIR);	#MD changed from: my @countsEdit = grep {/counts_edit$/} readdir(DIR);
