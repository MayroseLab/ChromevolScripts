...
sub main{
	...
	print LOG "$countsEdit\n";
	print LOG "$inTreesFile \n\n, $countsFile\n\n";
	...
	my $script = $FindBin::Bin."/run_inferences.pl";
...
sub send_to_queue{
	...
	my $stderrFile = "$outDir/";
	#my $stdoutFile = "$outDir/$jobName.OU";
	my $stdoutFile = "$outDir/";
	
	print($shFile);
	open (SH_SCRIPT,"> $shFile");
	print SH_SCRIPT '#!/bin/bash',"\n";
	#print SH_SCRIPT '#!/bin/tcsh',"\n";
	print SH_SCRIPT '#PBS -N ',"$jobName\n";
	#print SH_SCRIPT '#$ -N ',"$jobName\n";
	print SH_SCRIPT '#PBS -r y',"\n";
	#print SH_SCRIPT '#$ -S /bin/tcsh',"\n";
	print SH_SCRIPT '#PBS -q lifesciweb',"\n";
	print SH_SCRIPT '#PBS -v PBS_O_SHELL=bash,PBS_ENVIRONMENT=PBS_BATCH',"\n";
	#print SH_SCRIPT '#$ -cwd',"\n";
	print SH_SCRIPT '#PBS -e ',$stderrFile,"\n";
	print SH_SCRIPT '#PBS -o ',$stdoutFile,"\n";
	#print SH_SCRIPT "module load  perl/perl-5.16.3\n";
	print SH_SCRIPT "module load perl/perl-5.28.1\n";
	print SH_SCRIPT "$command\n";
	close(SH_SCRIPT);
	
	#system ("qsub -l itaym -p -2 $shFile") == 0
	#system ('ssh bioseq@powerlogin qsub '."$shFile") == 0
	system ('qsub '."$shFile") == 0
