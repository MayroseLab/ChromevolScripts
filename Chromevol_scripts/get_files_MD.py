...
TREES_DIR = '/groups/itay_mayrose/michaldrori/MSA_JmodelTree/output/'
...
STEP1_PATH = '/groups/itay_mayrose/michaldrori/MD_ChromEvol/Chromevol_scripts/step1_preparation.pl'
STEP1_LILACH_PATH = '/groups/itay_mayrose/michaldrori/MD_ChromEvol/Chromevol_scripts/step1_preparation_queue_lilach.pl'
...
def create_genus_dir(genus,output_dir,prune_flag):
	...
	"""
	if prune_flag==1:
		dir_path = output_dir  + genus + "/" + genus + "_Chromevol_special/"
	elif prune_flag == 0:
		dir_path = output_dir  + genus + "/" + genus + "_Chromevol_special/"
...
def get_trees(genus,output_dir):
	...
	files_dir = TREES_DIR + genus + '/concat/' # this is if we're taking trees BEFORE rnr
	#MAP_tree_file = files_dir + 'parsemb_map_tree.tre'
	trees_file = files_dir + 'parsemb_trees.tre'
	out_dir = output_dir  + genus + "/" + genus + "_Chromevol_special/"
	try:
...
def get_species_ToRemove(genus,output_dir):
	...
	species_remove_list_file = MSA_OUTPUT_PATH + genus + '/' + genus + '_Chromevol_special/remove_species_list.txt'
	f_species_to_remove = open(MSA_OUTPUT_PATH + genus + '/' + genus + '_Chromevol_special/remove_species_list.txt','w')
	# Get all species names with counts (according to counts file based on CCDB):
	counts_file = MSA_OUTPUT_PATH + genus + '/' + genus + '_Chromevol_special/' +genus + '.counts'
	...
	if (species_list_OnTree_count - removed_species_count) < 5:
		f_lessThanFive = open(MSA_OUTPUT_PATH + genus + '/' + genus + '_Chromevol_special/LessThanFiveWithCounts.txt','w')
	...
	outfile = MSA_OUTPUT_PATH + genus + '/' + genus + '_Chromevol_special/parsemb_trees.tre'
	...
	log_file = MSA_OUTPUT_PATH + genus + '/' + genus + '_Chromevol_special/pruneTree_log.txt'
...
def create_PIP_control(genus,output_dir,prune_flag):
	if prune_flag == 1:
		genus_dir = output_dir + genus + "/" + genus + "_Chromevol_special/"
	else:
		genus_dir = output_dir + genus + "/" + genus + "_Chromevol_special/"
...
if __name__ == "__main__":
	...
	output_dir = "/groups/itay_mayrose/michaldrori/MSA_JmodelTree/output/"
	with open(genera_list) as f:
		genera = f.readlines()
	for genus in genera:
		genus = genus.strip()
		working_dir_r="/groups/itay_mayrose/michaldrori/MSA_JmodelTree/output/" + genus + "/concat/"
		print(genus)
		...
		#Send to queue:
		chromEvol_dir=TREES_DIR + "/" + genus + "/" + genus + "_Chromevol_special/"
