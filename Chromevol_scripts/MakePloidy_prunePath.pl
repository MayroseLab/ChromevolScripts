...
foreach my $genus (@genera){
	...
	if ($qstatString =~ m/_$genus\_/){
		...
		#/groups/itay_mayrose/michaldrori/MSA_JmodelTree/output/Acanthus/Acanthus_Chromevol_prune
		$control_file = $working_dir.$genus."/".$genus."_Chromevol_prune/PIP_control";
		$status = check_status($genus,$working_dir);
		if($status eq "lessThan"){
			$status = "Less than 5 species with counts after Prune - chromevoldidn't run";
		}
		else{
			if ($status eq "ready"){
...
sub check_status{
	...
	my $status;
	my $curr_dir = $working_dir.$genus."/".$genus."_Chromevol_prune/chromevol_out/";
	chdir ($curr_dir);
	if (-e $working_dir.$genus."/".$genus."_Chromevol_prune/LessThanFiveWithCounts.txt"){
		$status = "lessThan";
		return($status);
	}
	if (-e "ploidy.csv"){
