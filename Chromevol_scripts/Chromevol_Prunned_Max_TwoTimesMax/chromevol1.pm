...
sub prepare_initial_run{
	...
	if ($root == 0){
		...
		if ($maxInData > $root){
			$maxChrNum = 1+(2*$maxInData);	# MD - changed from -> $maxChrNum = -1;
		}
		else{
			$maxChrNum = $root + 1;
		}
	...
	my @chromNums = @{calc_min_max_chrom_num($countFile,1)};
	my $maxInData = $chromNums[1];
	$maxChrNum = 1+(2*$maxInData);	# highest count in file + 1  # MD - changed from -> $maxChrNum = -1; #MD changed to 1+(2*$maxInData)
