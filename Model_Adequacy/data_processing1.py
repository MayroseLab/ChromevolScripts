...
def get_rate_parameters(res, params, d1, freq_file):
    ...
    for line in res:
        ...
        if line.startswith(params):
            ...
            if tmp:
                key = tmp.group(1)
                val = int(tmp.group(2)) if key == "DysBnumBER" else float(tmp.group(2))
...
def get_params(res_file, freq_file):
    ...
    """
    d1 = dict.fromkeys(["LOSS_CONST", "GAIN_CONST", "DUPL", "DysBnumBER_R", "DysBnumBER", "HALF_DUPL"], None)
    d2 = dict.fromkeys(["_lossConstR", "_gainConstR", "_duplConstR", "_baseNumberR", "_baseNumber", "_demiPloidyR"], None)
    mapping_dict = {"_lossConstR": "LOSS_CONST", "_gainConstR": "GAIN_CONST", "_duplConstR": "DUPL", "_baseNumberR": "DysBnumBER_R",  "_baseNumber": "DysBnumBER", "_demiPloidyR": "HALF_DUPL"}
    params = tuple(d1.keys())
    with open(res_file, "r") as res:
        d1 = get_rate_parameters(res, params, d1, freq_file)
    for key in d2:
        d2[key] = d1[mapping_dict[key]]
    is_demi = extract_line_from_file(res_file, "#Half_duplication rate is same as duplication rate")  # if DysDupDem* parameter needs to be updated
    if is_demi:
